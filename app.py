...
# TODO 364: Will need to create a database of this name. May also need to edit the database URL if your computer requires a password for you to run this.
...
tags = db.Table('tags', db.Column('search_id', db.Integer, db.ForeignKey(
    'searchterms.id')), db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')))
...
user_collection = db.Table('user_collection', db.Column('user_id', db.Integer, db.ForeignKey(
    'gifs.id')), db.Column('collection_id', db.Integer, db.ForeignKey('personalGifCollections.id')))
...
def get_or_create_search_term(term):
    ...
    if not searchTerm:
        ...
        for x in gif_list:
            gif = get_or_create_gif(x['title'], x['embed_url'])
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    gifCollection = PersonalGifCollection.query.filter_by(
        name=name, user_id=current_user.id).first()
    if not gifCollection:
        gifCollection = PersonalGifCollection(
